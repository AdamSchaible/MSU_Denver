...
-- Date: 11/08/2019
-- Name(s): Adam Schaible and Chris Rabka
--
...
SELECT DISTINCT prvState AS 'State', prvName AS 'Provider name'
    FROM Providers
    GROUP BY prvName
    ORDER BY prvName;

-- c) List the number of (distinct) providers.
SELECT COUNT(DISTINCT prvName) AS 'Number of distinct Providers'
    FROM Providers;
...
SELECT DISTINCT prvState AS State, COUNT(prvName) AS 'Distinct Number of Providers'
    FROM Providers
    GROUP BY prvState;

-- e) List the number of (distinct) hospital referral regions (HRR).
SELECT COUNT(DISTINCT hrrState, hrrName) AS 'Number of distinct HRRs'
    FROM HRRs;

-- f) List the number (distinct) of HRRs per state (also printing out the state).
SELECT DISTINCT hrrState AS State, COUNT(hrrName) AS 'Distinct Number of HRRs'
    FROM HRRs
    GROUP BY hrrState;
...
SELECT prvName AS 'Provider', prvState AS 'State', avgTotalPayments AS 'Average Charged Amount' 
    FROM ChargesAndPayments A 
        JOIN Providers B ON A.prvId = B.prvId 
        JOIN DRGs C ON A.drgCode = C.drgCode
    WHERE C.drgCode = '308'
    ORDER BY avgTotalPayments DESC LIMIT 10; 
...
SELECT prvState AS 'State', TRUNCATE(AVG(avgTotalPayments),2) AS 'Average Charged Amount'
    FROM ChargesAndPayments A 
        JOIN Providers B ON A.prvId = B.prvId 
        JOIN DRGs C ON A.drgCode = C.drgCode
    WHERE C.drgCode = '308'
...
SELECT prvName AS 'Hospital', drgDesc AS 'Clinical Condition'
    FROM ChargesAndPayments A
        JOIN Providers B ON A.prvId = B.prvId 
        JOIN DRGs C ON A.drgCode = C.drgCode
    ORDER BY (avgTotalPayments - avgMedicarePayments) DESC LIMIT 1;
